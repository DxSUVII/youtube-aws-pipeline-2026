...
def resolve_database(event: dict, env: dict | None = None) -> str:
    """Resolve the Glue/Athena database from event input or environment configuration."""
    env = env or os.environ
    return event.get("database") or env.get("GLUE_DB_SILVER") or env.get("SILVER_DATABASE") or "yt_pipeline_silver_dev"
...
def lambda_handler(event, context):
    ...
    """
    database = resolve_database(event)
